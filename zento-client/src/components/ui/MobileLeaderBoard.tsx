...
/> </Link>
              ...
              <Link href="/" className="hidden lg:block group relative ml-6">
                <span className="text-gray-300 font-medium transition-colors relative inline-block pb-1">
                  Explore
                  <span className="absolute bottom-0 transition-all duration-300 left-1/2 transform -translate-x-1/2 w-0 group-hover:w-3/4 h-[2px] bg-[#d5a514]"></span>
                </span>
              </Link>
      ...
      <div className="flex-1 max-w-6xl px-4 sm:mx-auto w-full py-6 mb-16 pb-32 sm:pb-8 mx-auto">
        ...
        {/* Stats Cards */}
        <div className="w-full flex gap-2 text-white p-2 rounded-lg mb-8 bg-[#1a191e] border border-gray-700/20">
          <div
            className={`flex-1 flex flex-col bg-[#0e0e0f]/50 p-3 sm:p-4 rounded-lg items-center justify-center space-y-2 border ${
              account?.address ? "border-gray-700/10" : "border-gray-700/5"
            }`}
          >
            <User width={20} height={20} className={account?.address ? "text-[#d5a514]" : "text-gray-700"} />
            <div className="text-center">
              <div className="text-[10px] sm:text-xs text-gray-400">
                {account?.address ? "You're Ranked" : "Your Rank"}
              </div>
              <div className={`text-lg sm:text-xl font-bold ${account?.address ? "text-[#d5a514]" : "text-gray-600"}`}>
                {account?.address ? `#${currentUserData?.rank || "-"}` : "---"}
              </div>
            </div>
          </div>

          <div className="flex-1 flex flex-col bg-[#0e0e0f]/50 p-3 sm:p-4 rounded-lg items-center justify-center space-y-2 border border-gray-700/10">
            <PixelCoins className="text-green-200" />
            <div className="text-center">
              <div className="text-[10px] sm:text-xs text-gray-400">Coins</div>
              <div className="text-lg sm:text-xl font-bold text-[#d5a514]">{currentUser?.points ?? 0}</div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col bg-[#0e0e0f]/50 p-3 sm:p-4 rounded-lg items-center justify-center space-y-2 border border-gray-700/10">
            ...
            <div className="text-center">
              ...
              <div className="text-lg sm:text-xl font-bold text-[#d5a514]">{currentLevel.level}</div>
            </div>
          </div>
        </div>

        {/* Tab Navigation */}
        <div className="bg-[#1a191e] border border-gray-700/20 rounded-md px-1 py-1 mb-8 flex-shrink-0">
            ...
            {[
              ...
              <button
                ...
                className={`flex-1 py-2 px-3 rounded text-sm font-medium transition-all duration-200 ${
                  activeTab === tab.key
                    ? "bg-[#d5a514] text-white border border-[#d5a514]"
                    : "bg-[#0e0e0f] text-gray-400 border border-gray-700/20 hover:text-white"
                }`}
              >
                {tab.label}
              </button>
        ...
        <div className="space-y-2">
          {currentData?.slice(0, 50).map((player: any) => (
            <div
              key={player.id}
              className={`flex items-center justify-between p-4 rounded-lg border transition-all duration-200 bg-[#1a191e] ${
                player.isCurrentUser
                  ? "border-[#d5a514] bg-[#d5a514]/10 ring-2 ring-[#d5a514]/30"
                  : "border-gray-700/20 hover:border-gray-700/40"
              }`}
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="text-[#d5a514] font-bold min-w-fit flex items-center justify-center gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div
                    className={`font-medium truncate text-sm ${player.isCurrentUser ? "text-[#d5a514]" : "text-white"}`}
                  >
                    {player.name}
                    {player.isCurrentUser && <span className="text-xs text-[#d5a514] ml-2">(You)</span>}
                  </div>
          ...
          {(!currentData || currentData.length === 0) && (
            <div className="text-center py-12 bg-[#1a191e] rounded-xl border border-gray-700/20">
              <div className="flex justify-center items-center gap-2">
                <div
                  className="w-2 h-2 bg-[#d5a514] rounded-full animate-bounce"
        ...
        {currentUserData && currentUserData.rank > 50 && (
          <div className="mt-8 bg-[#1a191e] border border-[#d5a514]/30 rounded-xl px-4 py-4 flex-shrink-0">
            <div className="text-center text-sm text-gray-400 mb-3">Your Rank</div>
            <div className="flex items-center justify-between p-4 rounded-lg border border-[#d5a514] bg-[#d5a514]/10">
              <div className="flex items-center gap-3">
                <Trophy className="w-5 h-5 text-[#d5a514]" />
                <div>
                  <div className="font-medium text-[#d5a514]">#{currentUserData.rank}</div>
